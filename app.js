...
// -------------------------------
// FORMATAGE AUTOMATIQUE MGRS
// -------------------------------

function formatMGRSInput(value) {
  const clean = value
    .toUpperCase()
    .replace(/[^A-Z0-9]/g, "");
  ...
  const zone = clean.slice(0, 3);
  const square = clean.slice(3, 5);

  let digits = clean.slice(5);

  if (digits.length > 10) {
    digits = digits.slice(0, 10);
  }

  const east = digits.slice(0, 5);
  const north = digits.slice(5, 10);
...
function setupMGRSFormatting(id) {
  ...
  input.addEventListener("input", () => {
    ...
    input.value = formatMGRSInput(input.value);
...
function hasCorrectMGRSPrecision(str) {
  const clean = str.replace(/\s+/g, "").toUpperCase();

  if (clean.length < 15) return false;

  const digits = clean.substring(5);
...
function setupMGRSFormatting(id) {
  ...
  console.log("Formatage activé pour :", id);

  input.addEventListener("blur", () => {

    console.log("BLUR DETECTE :", id);
...
if (modeOA === "flat") {
  ...
  const corrected = applyFlatCorrection(
    objStr,
    azMilsGrille,
    latCorr,
    rangeCorr
  ...
  const clean = objStr.replace(/\s+/g, "").toUpperCase();

  const prefix = clean.slice(0, 5);

  const xTxt = String(corrected.x).padStart(5, "0");
  const yTxt = String(corrected.y).padStart(5, "0");

  const mgrsNew = prefix + xTxt + yTxt;
  ...
  out.innerHTML =
    `<div class="oa-new-obj">
      📍 Nouvelle Obj : <strong>${mgrsNew}</strong>
    </div>`;

  showToast("Correction locale appliquée ✓", "success");
